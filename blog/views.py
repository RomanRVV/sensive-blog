...
def serialize_tag(tag):
    return {
        'title': tag.title,
        'posts_with_tag': len(Post.objects.filter(tags=tag)), # заменить на tag.posts__count
    }
...
def index(request):
    ...
    context = {
        'most_popular_posts': [
            serialize_post_optimized(post) for post in count_comments(most_popular_posts)
        ],
        'page_posts': [serialize_post_optimized(post) for post in count_comments(most_fresh_posts)],
